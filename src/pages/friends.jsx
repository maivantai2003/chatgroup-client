...
const FriendRequest = ({ id, friendId, userId, userName, avatar }) => {
  ...
  const handleRejectFriend = async (id, friendId, userId) => {
    let friendDto = { userId, friendId, status: 3 };
    try {
      var result = await dispatch(
        UpdateFriend({ id, friendDto })
      ).unwrap();
      if (!result) toast.error("Hủy kết bạn không thành công");
    } catch (ex) {
      toast.error(ex);
    }
  };
  ...
  const handleAcceptFriend = async (id, friendId, userId, avatar, userName) => {
    let friendDto = { userId, friendId, status: 1 };
    try {
      var result = await dispatch(UpdateFriend({ id, friendDto }));
      if (result) {
        // tạo hội thoại cho 2 bên
        ...
        var resultConversationUser = await dispatch(
          CreateConversation(conversationDtoUser)
        ).unwrap();

        if (resultConversationUser) {
          var resultConversationFriend = await dispatch(
            CreateConversation(conversationDtoFriend)
          ).unwrap();

          if (!resultConversationFriend) {
            toast.error("Lỗi tạo cuộc trò chuyện");
            return;
          }
          if (connection) {
            await connection
              .invoke("AcceptFriend", userId.toString(), resultConversationFriend)
              .catch((err) => console.error("Error calling AcceptFriend:", err));
          }
        }
      }
  ...
  return (
    <div className="bg-white shadow-lg rounded-xl p-4 flex items-center justify-between hover:shadow-xl transition">
      ...
      <div className="flex gap-2">
        <button
          className="px-4 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg transition"
          onClick={() => handleRejectFriend(id, friendId, userId)}
        >
          Từ chối
        </button>
        <button
          className="px-4 py-1.5 bg-blue-500 hover:bg-blue-600 text-white rounded-lg transition"
...
const FriendSuggestion = ({ userId, userName, avatar, id }) => {
  ...
  const handleRequestFriend = async (userId, id) => {
    setLoading(true);
    let friendDto = { userId: id, friendId: userId, status: 0 };
    try {
      var result = await dispatch(AddFriend(friendDto));
      if (result) {
        if (connection) connection.invoke("SendRequestFriend", userId.toString());
  ...
  return (
    <div className="bg-white shadow-md rounded-xl p-4 flex flex-col items-center hover:shadow-lg transition">
      ...
      <div className="flex gap-2 mt-3">
        <button className="px-4 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-600 rounded-lg transition">
          Bỏ qua
        </button>
        <button
          className="px-4 py-1.5 bg-blue-500 hover:bg-blue-600 text-white rounded-lg transition"
...
const FriendSuggestions = ({ id }) => {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <h2 className="text-2xl font-bold mb-4 text-gray-800">📩 Lời mời kết bạn</h2>
      <div className="mb-8 overflow-y-auto max-h-80 space-y-4">
        {loading ? (
